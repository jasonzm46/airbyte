...
class CdcPartitionReader<T : Comparable<T>>(
    ...
    val dbzOps: DebeziumOperations<T>,
    val upperBound: T,
    ...
    private val closeReasonReference = AtomicReference<CloseReason>()
    ...
    override fun checkpoint(): PartitionReadCheckpoint {
        ...
        return PartitionReadCheckpoint(dbzOps.serialize(output), numRecords.get())
    }
    ...
    inner class EventConsumer(
        ...
        override fun accept(event: ChangeEvent<String?, String?>) {
            ...
            val debeziumRecord: DebeziumRecord? =
                if (event.value() == null) {
                    ...
                    DebeziumRecord(Jsons.readTree(event.value()))
                }
            val isRecord: Boolean
            // Process records, ignoring heartbeats which are only used for completion checks.
            if (debeziumRecord != null && !debeziumRecord.isHeartbeat) {
                isRecord = true
                val airbyteRecord: AirbyteRecordMessage =
                    dbzOps.toAirbyteRecordMessage(debeziumRecord)
            ...
            val closeReason: CloseReason? = run {
                ...
                val currentPosition: T =
                    sourceRecord?.let(dbzOps::position)
                        ?: debeziumRecord?.let(dbzOps::position) ?: return@run null
